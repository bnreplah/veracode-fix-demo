...
package_archive() {
    ...
    if [ "$package" == "true" ]; then # meant for inline execution
        if [[ "$DEBUG" == "true" ]]; then
    ...
    if [ -z "$out_dir" ]; then
        ...
        if [ "$prompt" == "true" ]; then
            read -p "Please enter an output directory:" outdirResponse
            read -p "Please enter a source matching the type:" sourceResponse
            read -p "Please enter the type, directory or repo:" typeResponse
            out_dir="${outdirResponse}"
            ...
            echo "Setting packager defaults"
            out_dir="./"
            source="./"
            type="directory"
            echo "Outdir: $out_dir "
    ...
    if [ -z "$source" ]; then
        echo "[ERROR]: Please provide a source"
    fi
    ...
    if [ -n $trust ] && [ "$trust" == "true" ]; then
        if [[ "$DEBUG" == "true" ]]; then
            ...
            if [ "$out_dir" == "./" ]; then
                if $auto_package; then
                    artifact_path="$( remove_trailing_period $( veracode package --output "$out_dir" --source "$source" --type "$type" --trust --debug | grep "Copied artifact:" | cut -d ':' -f3 ) )"
                    echo Artifact Path: "$artifact_path"
                else
                    veracode package --output "$out_dir" --source "$source" --type "$type" --trust --debug
                    echo "Please prvovide the artifact file path"
                    read -p "The Artifact Path: " artifact_path
                fi
                
            else # if not the current working directory print out the file with the directory as the artifact 
                if $auto_package; then
                    artifact_path="${out_dir}$( remove_trailing_period $( veracode package --output "$out_dir" --source "$source" --type "$type" --trust --debug  | grep "Copied artifact:" | cut -d ':' -f3 ) )"
                    echo "Artifact Path: $artifact_path"
                else
                    veracode package --output "$out_dir" --source "$source" --type "$type" --trust --debug 
                    echo "Please prvovide the artifact file path"
            ...
            if [ "$out_dir" == "./" ]; then
                if $auto_package; then
                    ...
                    veracode package --output "$out_dir" --source "$source" --type "$type" --trust
                    echo "Please prvovide the artifact file path"
                    read -p "The Artifact Path: " artifact_path
                fi
                #echo Artifact Path: "$artifact_path"
        ...
        if [[ "$DEBUG" == "true" ]]; then
            echo "[DEBUG=$DEBUG]:: Trust is set to $trust "
            if [ "$out_dir" == "./" ]; then
                veracode package --output "$out_dir" --source "$source" --type "$type"  --debug
                echo "Please prvovide the artifact file path"
                read -p "The Artifact Path: " artifact_path
                #artifact_path="$( remove_trailing_period $( veracode package --output "$out_dir" --source "$source" --type "$type"  --debug  | grep "Copied artifact:" | cut -d ':' -f3 ) )"
                #echo "Artifact Path: $artifact_path"
            elif [ "$out_dir" == "." ]; then
                $out_dir = "./"
                veracode package --output "$out_dir" --source "$source" --type "$type" --debug 
                ...
                veracode package --output "$out_dir" --source "$source" --type "$type"  
                echo "Please prvovide the artifact file path"
                read -p "The Artifact Path: " artifact_path
...
main() {
    ...
    if [ "$package" == "true" ]; then
        ...
        echo "Packaging the application and exiting"
        package_archive    
        ls $out_dir
        exit 0
    fi
    ...
    while true; do
        ...
        echo "Processing results from the scan"
        ...
        if [ "$response_lower" == "yes" ] || [ "$response_lower" == "y" ]; then
            ...
            if [ "$response_lower_" == "yes" ] || [ "$response_lower_" == "y" ]; then
                ...
                if [ -z "$artifact_path" ]; then
                    echo "Failed to retrieve the artifact path automatically"
                    ls $out_dir
